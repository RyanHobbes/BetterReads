-- 1. Creates a stored procedure to select Reviews ID and Bookstitle by joining Reviews and Bookstables.
DELIMITER $$
CREATE PROCEDURE GetReviewsAndBooks()
BEGIN
    SELECT Reviews.ID, Book.title
    FROM Reviews
    JOIN BooksON Reviews.title = Book.title;
END $$
...
-- 4. Creates a stored procedure to retrieve the image of a Booksby its title.
DELIMITER $$
...
BEGIN
    SELECT b.Title FROM Booksb, Publisher p
...
-- 11. Creates a stored procedure to retrieve all books from the Bookstable.
DELIMITER $$
